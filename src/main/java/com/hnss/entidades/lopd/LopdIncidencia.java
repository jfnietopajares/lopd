...
//http://localhost:8080/lopd/?APL=jjj&USR=06551256M&NOM=ASDFASDFADFAFADFADFA&MAI=jnieto@saludcastillayleon.es&ADDR=12.12.12.12&TIME=&NHC=29675&PAC=ASDFASDFADFASDFASFASDFAFASDF
	...
	public String getHtmlContenidoRegistro() {
		DateTimeFormatter fechaFormato = DateTimeFormatter.ofPattern("dd/mm/YYYY hh:mm");
		String textHtml = "Número:" + this.getId() + "\n";
		textHtml = textHtml.concat("Fecha hora: " + fechaFormato.format(this.getFechaHora()) + " \n");
		textHtml = textHtml.concat("Tipo: " + this.getTipo().getDescripcion() + " \n");
		if (this.getPaciente() != null && this.getPaciente().getNumerohc() != null
				&& !this.getPaciente().getNumerohc().isEmpty()) {
			textHtml = textHtml.concat("Númerohc: " + this.getPaciente().getNumerohc() + " \n");
			textHtml = textHtml.concat("Paciente: " + this.getPaciente().getApellidosnombre() + " \n");
			textHtml = textHtml.concat("Documento: " + this.getDescriDocu() + " \n");
			textHtml = textHtml.concat("Documento fecha: " + " " + " \n");

		} else {

		}
		textHtml = textHtml.concat("Descripción: " + this.getDescripcionError() + " \n");
		return textHtml;
	}
