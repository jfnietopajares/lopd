...
public class LopdIncidenciaRespuesta extends VerticalLayout {
	...
	private HorizontalLayout fila11 = new HorizontalLayout();
	...
	public LopdIncidenciaRespuesta(LopdIncidencia inciParam) {
		...
		this.addComponents(fila1, fila2, fila3, fila4, fila5, fila6, fila7, fila8, fila9,contenedorBotonesAyuda, contenedorBotonesAccion, fila11);
...
// datos para incidencia de usuario
		...
		jimenaBorraInf.addClickListener(e -> jimenaBorrarClick());
		...
		fila11.addComponent(gridNotas);
		doFilaDocumentos();
		doActivaBotones();
		doActualizaGrid();
	}

        public void jimenaBorrarClick(){
            new Notificaciones(paciente.getApellidosnombre());
            FrmJimenaBorraInf jimenaBorrarInf = new FrmJimenaBorraInf(paciente.getNumerohc(), null);
            VentanaFrm ventanaFrm= new VentanaFrm(this.getUI(), jimenaBorrarInf, "Borrar informe de jimena");
			...
			if (new LopdIncidenciaDAO().actualizaRespuesta(incidencia) == false) {
				...
				if (incidencia.getResuelta() == true) {
					...
					new MandaMail().sendEmail(incidencia.getUsuarioRegistra().getMail(),
							...
							LopdIncidencia.MAIL_CONTENIDO_CABECERA + incidencia.getHtmlContenidoRegistro() + "\n\n"
									+ incidencia.getHtmlContenidoSolución());
				}
				doLimpiaDatos();
			}
